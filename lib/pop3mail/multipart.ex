defmodule Pop3mail.Multipart do
  ...
  """
   ...
   def parse_content(%Part{} = multipart_part) do
   ...
   def is_multipart?(%Part{} = multipart_part) do
   ...
   * `lines` - part content splitted in lines
   ...
   def parse_part_decode(%Part{} = multipart_part, encoding, lines) when is_binary(encoding) and is_list(lines) do
   ...
   * `list` - lines
   ...
   def parse_part_lines(%Part{} = multipart_part, encoding, []) when is_binary(encoding) do
   ...
   def lines_continued(line1, []) when is_binary(line1), do: {line1, []}
   ...
   * `list` - lines
   ...
   def parse_part_content_type(%Part{} = multipart_part, encoding, [line | otherlines]) do
   ...
   def parse_content_type(%Part{} = multipart_part, content_type) when is_binary(content_type) do
   ...
   * `content_type_parameters` - list of parameters in the format key=value
   ...
   def parse_content_type_parameters(%Part{} = multipart_part, content_type_parameters) when is_list(content_type_parameters) do
       ...
       multipart_part =
          if String.length(media_type) > 0 do
             %{multipart_part | media_type: media_type}
          else
             multipart_part
          end
   ...
   defp set_boundary(%Part{} = multipart_part, boundary_keyval) do
      if StringUtils.contains?(boundary_keyval, "=") do
   ...
   defp set_charset(%Part{} = multipart_part, charset_keyval) do
      if StringUtils.contains?(charset_keyval, "=") do
   ...
   * `list` - lines
   ...
   def parse_part_content_id(%Part{} = multipart_part, encoding, [line | otherlines]) do
       content_id = line
   ...
   * `list` - lines
   ...
   def parse_part_content_location(%Part{} = multipart_part, encoding, [line | otherlines]) do
   ...
   * `list` - lines
   ...
   def parse_part_transfer_encoding(%Part{} = multipart_part, _, [line | otherlines]) do
       encoding = line
   ...
   * `list` - lines
   ...
   def parse_part_skip(%Part{} = multipart_part, encoding, [line | otherlines]) do
   ...
   * `list` - lines
   ...
   def parse_part_unknown_header(%Part{} = multipart_part, encoding, [line | otherlines]) do
   ...
   * `list` - lines
   ...
   def parse_part_disposition(%Part{} = multipart_part, encoding, [line | otherlines]) do
   ...
   def parse_disposition(%Part{} = multipart_part, disposition) when is_binary(disposition) do
   ...
   * `disposition_parameters` - list of parameters in the format key=value
   ...
   def parse_disposition_parameters(%Part{} = multipart_part, disposition_parameters) when is_list(disposition_parameters) do
   ...
   def decode(encoding, text) when is_binary(encoding) and is_binary(text) do
   ...
   def decode_base64!(text) when is_binary(text) do
     try do
   ...
   def extract_and_set_filename(%Part{} = multipart_part, content_parameters, parametername) when is_list(content_parameters) and is_binary(parametername) do
       ...
       if length(name_parts) > 0 do
          extract_and_set_filename_from_name_parts(multipart_part, name_parts)
       else
          multipart_part
       end
   end
   ...
   defp extract_and_set_filename_from_name_parts(%Part{} = multipart_part, name_parts) do
      charset = ""
      ...
      if String.length(filename) > 0 do
         ...
         if String.length(charset) > 0 do
            %{multipart_part | filename_charset: charset}
         else
            multipart_part
         end
      else
   ...
   defp sort_name_parts(name_parts) do
      if length(name_parts) > 1 do
         Enum.sort(name_parts, fn({a, _, _}, {b, _, _}) -> a <= b end)
      else
         name_parts
      end
   end
   ...
   defp decode_filename_and_charset(filename, charset, with_charset) do
     if with_charset do
        ...
        if String.length(decoded_filename) > 0 do
           {decoded_filename, decoded_charset}
        else
           {filename, charset}
        end
     else
       ...
       if String.contains?(filename, "=?") do
          decoded_word_filename_and_charset(filename)
       else
          {filename, charset}
       end
     end
